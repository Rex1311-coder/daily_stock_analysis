...
class TechnicalScreener:
    ...
    def batch_calculate(self, stock_list: List[Dict]) -> List[Dict]:
        if not stock_list:
            return []
        logger.info(f"开始批量计算技术指标，共 {len(stock_list)} 只")
        results = []
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self._calc, stock): stock for stock in stock_list}
        ...
        logger.info(f"技术指标计算完成: {len(results)} 只")
        return results
    ...
    def _calc(self, stock: Dict) -> Dict:
        ...
        try:
            ...
            ma60 = self._ma(close, 60)[-1]
            
            is_bullish = ma5 > ma10 > ma20
            is_strong = ma5 > ma10 > ma20 > ma60
            
            ma_score = 30 if is_strong else (20 if is_bullish else (10 if latest_price > ma20 else 0))
